...
struct ArbLatticeConnectivity {
    ...
    ArbLatticeConnectivity(size_t chunk_begin_, size_t chunk_end_, size_t num_nodes_global_, size_t Q_)
        : chunk_begin(chunk_begin_),
          ...
          cuts(std::make_unique<int[]>(26 * (chunk_end_ - chunk_begin_))) {
    ...
    int& cut_distance(size_t d, size_t local_node_ind) { return cuts[local_node_ind + d * getLocalSize()]; }
    int cut_distance(size_t d, size_t local_node_ind) const { return cuts[local_node_ind + d * getLocalSize()]; }
};
